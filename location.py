...
def location_pipe(uri, batch_size, sources):
    """
    ...
    locs = Location(uri, batch_size, sources)
    locs.run()
...
class Location():
    ...
    def location_start(self):
        ...
        for _doc in self.list_docs:
            try:
                combined_text += [(_doc['_id'], self.combine_text(_doc['title_translated'], self.fix_maintext(_doc['maintext_translated'])))]
            except KeyError:
                combined_text += [(_doc['_id'], _doc['title_translated'])]
...
def main():
    ...
    db = MongoClient(uri).ml4p
    source_domains = db.sources.distinct('source_domain', filter={'include' : True, 'primary_location' : {'$in' : ['KEN']}})
    # source_domains = db.sources.distinct('source_domain', filter={'include' : True, 'source_domain' : {'$in' : ['africanews.com']}})
    ...
    location_pipe(uri,64,source_domains)
