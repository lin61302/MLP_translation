...
def panoramacomal_story(soup):
    ...
    #maintext
    try:
        maintext_body = soup.find("div", {"style": "float:left; width:500px"})
        maintext = maintext_body.findAll('p')
        finaltext = ''
        for paragraph in maintext:
          paragraph_final = paragraph.text.strip()
          finaltext += paragraph_final
        hold_dict['maintext'] = finaltext
    except: 
        maintext = None
    ...
    try:
        ...
        article_date_text = soup.find('div', {'class':'meta-info'}).text.strip()
        article_day_text = article_date_text.split('|')[0].strip()
        article_hour_text =  article_date_text.split('|')[1].strip()
        article_text_final = article_day_text + ' ' + article_hour_text
        print(article_text_final)
        article_date = dateparser.parse(article_text_final, date_formats=['%b %d, %Y %H:%M'])
...
def telegrafal_story(soup):
    ...
    #date
    try:
        date_publish_raw = soup.find("time", {"class": "entry-date updated td-module-date"})
        print(date_publish_raw)
        date_final = dateparser.parse(date_publish_raw['datetime'], settings={'RETURN_AS_TIMEZONE_AWARE': False})
        hold_dict['date_publish'] = date_final
...
def heraldcozw_story(soup):
    """
    Function to pull the information we want from Herald.co.zw stories
    :param soup: BeautifulSoup object, ready to parse
    """
    hold_dict = {}
    #title
    ...
    #date
    try:
        article_date = soup.find('time', attrs={"class": "hide-microdata updated"})
        date = article_date['datetime']
        hold_dict['date_publish'] = dateparser.parse(date)
    except:
        article_date = None
...
#%%
def jamaicagleanercom_story(soup):
    """
    Function to pull the information we want from jamaicagleaner.com stories
    :param soup: BeautifulSoup object, ready to parse
    """
    # create a dictionary to hold everything in
    hold_dict = {}
    ...
    # Get Main Text:
    try:
        maintext = soup.find("strong").text
        hold_dict['maintext'] = maintext
    ...
    #date
    try:
        article_date = soup.find('span', attrs={'class':'jg-published-created'}).text
        date = article_date.split('|')[1].strip()
        hold_dict['date_publish'] = dateparser.parse(date)
    except:
        article_date = None
        hold_dict['date_publish'] = None
