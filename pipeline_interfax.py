    ...
    try:
        ...
        try:
            ...
            for i in txt.find_all('span'):
                date+=i.text+ ' '
            date = dateparser.parse(date,settings={'DATE_ORDER':'DMY'}).replace(tzinfo = None)
...
def collect_urls_sitemap(num):  
    ...
    '''
    urls=[]
    url = f"https://interfax.com.ua/sitemap-articles.xml?p={num}"
...
def collect_urls_keyword(keyword, page):
    ...
    try:
        #### change url pattern
        page_url = f'https://www.newtimes.co.rw/search/site/{keyword}?page={page}'
        page_req = requests.get(page_url,headers=headers)
        page_soup = BeautifulSoup(page_req.content, features="lxml")
        for i in page_soup.find('ol', {'class':"search-results apachesolr_search-results"}).find_all('a'):
            link = i['href']

            if 'newtimes.co.rw/' in link:
...
def pipeline(sitemap,
    ...
    if keyword:
        ...
        for keyword_tuple in keywords:
            ...
            for page in range(635,num_page):
                #collect urls
